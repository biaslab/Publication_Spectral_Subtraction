...
function write_wfb_ablation_table(dfs::Dict{String,DataFrame})
    io = IOBuffer()

    u_means = [overall_stats(dfs["SEM (uFB)"], m)[1] for m in METRICS]
    u_stds  = [overall_stats(dfs["SEM (uFB)"], m)[2] for m in METRICS]
    w_means = [overall_stats(dfs["SEM (WFB)"], m)[1] for m in METRICS]
    w_stds  = [overall_stats(dfs["SEM (WFB)"], m)[2] for m in METRICS]
    deltas  = w_means .- u_means

    fmt_delta(x) = (x ≥ 0 ? "+" : "-") * @sprintf("%.2f", abs(x))

    println(io, raw"% Auto-generated by scripts/generate_latex_tables.jl — do not hand-edit.")
    println(io, raw"% Regenerate with: julia --project=. scripts/generate_latex_tables.jl")
    println(io, raw"\begin{table}[h]")
    println(io, raw"\centering")
    println(io, raw"\caption{WFB ablation: SEM at $\alpha = 0.5$ (WFB) versus $\alpha = 0$ (uFB), with all other parameters as in Table~\ref{tab:parameters}. Values are sample mean $\pm$ one sample standard deviation over the 824-file VoiceBank+DEMAND test set.}")
    println(io, raw"\label{tab:wfb-ablation}")
    println(io, raw"\small")
    println(io, raw"\begin{tabular}{l|rrrr}")
    println(io, raw"\hline")
    println(io, raw"System & PESQ & CSIG & CBAK & COVL" * " \\\\")
    println(io, raw"\hline")

    # SEM (uFB) — mean row + ± std row
    @printf(io, "SEM (uFB, \$\\alpha=0\$)   & %s & %s & %s & %s \\\\\n",
            fmt2(u_means[1]), fmt2(u_means[2]), fmt2(u_means[3]), fmt2(u_means[4]))
    @printf(io, "                        & {\\small \$\\pm\\,%s\$} & {\\small \$\\pm\\,%s\$} & {\\small \$\\pm\\,%s\$} & {\\small \$\\pm\\,%s\$} \\\\\n",
            fmt2(u_stds[1]), fmt2(u_stds[2]), fmt2(u_stds[3]), fmt2(u_stds[4]))
    println(io, raw"\hline")

    # SEM (WFB) — mean row + ± std row
    @printf(io, "SEM (WFB, \$\\alpha=0.5\$) & %s & %s & %s & %s \\\\\n",
            fmt2(w_means[1]), fmt2(w_means[2]), fmt2(w_means[3]), fmt2(w_means[4]))
    @printf(io, "                        & {\\small \$\\pm\\,%s\$} & {\\small \$\\pm\\,%s\$} & {\\small \$\\pm\\,%s\$} & {\\small \$\\pm\\,%s\$} \\\\\n",
            fmt2(w_stds[1]), fmt2(w_stds[2]), fmt2(w_stds[3]), fmt2(w_stds[4]))
    println(io, raw"\hline")

    # Δ row
    @printf(io, "\$\\Delta\$                & \$%s\$ & \$%s\$ & \$%s\$ & \$%s\$ \\\\\n",
            fmt_delta(deltas[1]), fmt_delta(deltas[2]), fmt_delta(deltas[3]), fmt_delta(deltas[4]))
    println(io, raw"\hline")
    println(io, raw"\end{tabular}")
    println(io, raw"\end{table}")
...
function main()
    ...
    outputs = Dict(
        ...
        "tab_wfb_ablation.tex"           => write_wfb_ablation_table(dfs),
    )
