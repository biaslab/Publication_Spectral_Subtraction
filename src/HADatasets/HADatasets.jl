...
"""
    evaluate_audio_metrics(reference, processed; include_composite=false)

Evaluate all metrics for a reference-processed audio pair.

Accepts file paths, `Vector{Float64}`, or `SampleBuf` inputs. Returns a
NamedTuple with PESQ, SIG, BAK, and OVRL scores. When `include_composite=true`
and `pysepm` is importable via the `composite_wrapper` Python module, the
returned NamedTuple additionally carries the Hu & Loizou (2008) CSIG, CBAK,
and COVL composite scores.
...
function evaluate_audio_metrics(reference::AbstractString, processed::AbstractString;
    ...
    return Metrics.evaluate_all(ref_vec, proc_vec, ref_fs; include_composite=include_composite)
end
...
function evaluate_audio_metrics(reference::Vector{Float64}, processed::Vector{Float64}, fs::Int=16000;
    ...
    return Metrics.evaluate_all(reference, processed, fs; include_composite=include_composite)
end
...
function evaluate_audio_metrics(reference::SampleBuf, processed::SampleBuf;
    ...
    return Metrics.evaluate_all(ref_vec, proc_vec, fs; include_composite=include_composite)
end
